...
def _reachability_deadlock(state: GameState) -> bool:
    """
    For each unplaced box, check if there exists at least one free target
    that the box could theoretically reach (ignoring other boxes, just walls).
    If any box has zero reachable targets, it's a deadlock.
    Uses BFS from each box position treating walls as obstacles.
    """
    ...
    free_tgts  = set(t for t in targets if t not in state.boxes)
    ...
    def can_reach_any_target(box: tuple) -> bool:
        # BFS — box can slide in any direction until hitting a wall
        # (simplified: just check if any free target is reachable through non-wall cells)
        visited = {box}
        queue   = deque([box])
        while queue:
            r, c = queue.popleft()
            ...
            for dr, dc in ((-1,0),(1,0),(0,-1),(0,1)):
                ...
                if (nr, nc) not in visited and \
                   0 <= nr < state.rows and 0 <= nc < state.cols and \
                   state.grid[nr][nc] != '#':
                    visited.add((nr, nc))
                    queue.append((nr, nc))
        return False

    for box in off_target:
        if not can_reach_any_target(box):
            return True
    return False
...
def has_deadlock(state: GameState) -> bool:
    ...
    Order: cheapest / broadest first.
      0. Dead zone      — O(1) lookup
      1. Corner         — O(boxes)
      2. Freeze         — O(boxes) amortised
      3. Line           — O(rows*cols + boxes)
      4. Reachability   — O(boxes * cells) BFS per box
    """
    ...
    if _line_deadlock(state):
        return True
    if _reachability_deadlock(state):
        return True
    return False
...
def deadlock_type(state: GameState) -> str | None:
    ...
    if _reachability_deadlock(state):
        return "Reachability deadlock — box cannot reach any target"
    return None
